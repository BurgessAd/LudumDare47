    ...
    private void OnGUI()
    {
        ...
        if (GUILayout.Button("Generate Grass Objects"))
        {
            ...
            else
            {
                ...
                float srcXPosition = objectBounds.center.x - (targetObjectType == 2 ? objectBounds.extents.x : 0f);
                float srcZPosition = objectBounds.center.z - (targetObjectType == 2 ? objectBounds.extents.z : 0f);
